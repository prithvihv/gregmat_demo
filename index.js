...
const quizFactory = [
    {
        "type": "single",
        "question": (med, eleArr) => `${eleArr.join(", ")}\nThe median of the list of positive integers above is ${med}. Which of the following could be the average of the list?`,
        "answers": [
            (correctoption, ...elarr) => correctoption * 4 - elarr.reduce((a, b) => (a + b)),
            (correctoption, ...elarr) => correctoption,
            (correctoption, ...elarr) => Math.round(Math.max(...elarr) * .75),
            (correctoption, ...elarr) => Math.round(Math.max(...elarr) * .75) + 0.5,
            (correctoption, ...elarr) => elarr.sort((x, y) => y - x)[Math.round(Math.random()) + 1] + 0.25
        ],
        "entered": [],
        "correct": elarr => median(elarr), // mean is the same
        "question_gen": function () {
            console.log("question gen")
            let question = {
                "type": "single",
                "entered": []
            }
            let median = Math.round(Math.random() * 100) / 2
            if (median < 5) {
                return this.question_gen()
            }
            let elarr = ((median) => {
                let otherOptionWeight = median * 2
                let lowerOption = Math.ceil(median * .55)
                let upperLimit = Math.ceil(median * 1.45)
                let balance = otherOptionWeight - (lowerOption + upperLimit)
                upperLimit += balance
                let remaining = 4 * median - (lowerOption + upperLimit)
                let divideInto2 = Math.ceil(remaining * 0.40)
                let option1 = remaining - divideInto2
                let option2 = divideInto2
                return [lowerOption, upperLimit, option1, option2]
            })(median)
            eleArr = elarr.sort((x, y) => x - y)
            console.log(elarr, (eleArr[1] + eleArr[2]) / 2)
            let correctoption = this.correct(elarr)
            question["correct"] = correctoption
            let optionElarr = JSON.parse(JSON.stringify(shuffleArray(elarr)))
            let smallestElem = Math.min(...elarr)
            let indexOfAnswer = optionElarr.findIndex(x => smallestElem == x)
            optionElarr[indexOfAnswer] = 'x'
            question["question"] = this.question(median, optionElarr)
            // console.log(this.question(median, optionElarr))
            optionElarr.splice(indexOfAnswer, 1)
            // console.log(optionElarr)
            optionElarr = optionElarr.sort((x, y) => x - y)
            question["answers"] = []
            this.answers.forEach(optionGen => {
                question["answers"].push(optionGen(correctoption, ...optionElarr))
            });
            if (uniq(question["answers"]).length != question["answers"].length) {
                console.log(question["answers"])
                return this.question_gen()
            }

            // check for single answer only
            for (let i = 0; i < question["answers"].length; i++) {
                let ans = question["answers"][i]
                let neededNum = 4 * ans - (optionElarr.reduce((x, y) => x + y))
                if (ans != question["correct"] && neededNum > 0 && neededNum <= optionElarr[0]) {
                    console.log(`reconstructing because if answer ${ans} fits when x is ${neededNum}, but we expected ${question["correct"]} `)
                    return this.question_gen()
                }
            }

            question["answers"] = question["answers"].sort((x, y) => x - y)
            return question
        }
    }
]
...
const init = () => {
    quizFactory.forEach((q) => quiz.questions.push(q.question_gen()))
    cr_ContinueButton()
...
const cr_ContinueButton = () => {
    ...
    continueBUTTON.onclick = function () {
        cr_QuestionFeedback(checkIfCorrect())
    }
}

// Generate New Question Button
const cr_NewQuestion = () => {
    let newQuestionDIV = document.createElement(`div`)
    let newQuestionBUTTON = document.createElement(`button`)
    let newQuestionSPAN = document.createElement(`span`)
    newQuestionDIV.id = `new-question-button-contianer`
    newQuestionDIV.className = `quiz-continue-button-container`
    newQuestionBUTTON.className = `quiz-continue-button`
    newQuestionSPAN.classname = `quiz-continue-text`
    newQuestionSPAN.id = `new-question-text`
    newQuestionBUTTON.innerHTML = `Try New Question`
    newQuestionDIV.append(newQuestionBUTTON, newQuestionSPAN)
...
// Generate New Question Button
const cr_NextQuestion = () => {
    let nextQuestionDIV = document.createElement(`div`)
    let nextQuestionBUTTON = document.createElement(`button`)
    let nextQuestionSPAN = document.createElement(`span`)
    nextQuestionDIV.id = `next-question-button-contianer`
    nextQuestionDIV.className = `quiz-continue-button-container`
    nextQuestionBUTTON.className = `quiz-continue-button`
    nextQuestionSPAN.classname = `quiz-continue-text`
    nextQuestionSPAN.id = `next-question-text`
    nextQuestionBUTTON.innerHTML = `Try Similar Question`
    nextQuestionDIV.append(nextQuestionBUTTON, nextQuestionSPAN)
    // Moves to next question on click
    nextQuestionBUTTON.onclick = function () {
        loadNewQuestion(`next-question-load`)
    }
    return nextQuestionDIV
}

const cr_QuestionFeedback = (feedback) => {
    let parent = document.getElementById(`quiz-question-container`)
    parent.removeChild(parent.lastElementChild)
    parent.append(cr_QuestionFeedbackText(feedback), cr_NewQuestion())
    // parent.append(cr_QuestionFeedbackText(feedback), cr_NewQuestion(), cr_NextQuestion())

}

const cr_QuestionFeedbackText = (feedback) => {
    let feedbackSPAN = document.createElement("span")
    feedbackSPAN.className = `quiz-continue-text`
    feedbackSPAN.innerHTML = feedback
    feedbackSPAN.style.fontSize = "20px"
    feedbackSPAN.style.marginLeft = "0px"
    feedbackSPAN.style.marginBottom = "15px"
...
const asciiToIndex = ascii => ascii - 65

// Confirms if the selected answer was correct upon submission
const checkIfCorrect = () => {
    let question = quiz.questions[currentQuestionIndex]
    if (question.answers[asciiToIndex(question.entered[0])] == question.correct) {
        console.log("Right")
        return "Correct!"
    } else {
        console.log("Wrong")
        return "Incorrect. The correct answer was <b>" + question.correct + "</b>."
    }
}
